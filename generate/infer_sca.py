...
from utils import check_novelty, sample, canonic_smiles, sample_scaffold
...
if __name__ == '__main__':
    ...
    tempera = 1.1  # guaca 0.7 moses 1.1
    ...
    if 'guacamol' in args.data_name:
        prop2value = {'qed': [0.3, 0.5, 0.7], 'sas': [2.0, 3.0, 4.0], 'logp': [2.0, 4.0, 6.0],
                      ...
                      'sas_logp': [[2.0, 2.0], [2.0, 4.0], [2.0, 6.0], [3.0, 2.0], [3.0, 4.0], [3.0, 6.0], [4.0, 2.0],
                                   [4.0, 4.0], [4.0, 6.0]],
        ...
        prop2value = {'qed': [0.6, 0.725, 0.85], 'sas': [2.0, 2.75, 3.5], 'logp': [1.0, 2.0, 3.0],
                      'tpsa': [30.0, 60.0, 90.0],
                      ...
                      'tpsa_logp_sas': [[40.0, 1.0, 2.0], [40.0, 1.0, 3.5], [40.0, 3.0, 2.0], [40.0, 3.0, 3.5],
                                        [80.0, 1.0, 2.0], [80.0, 1.0, 3.5], [80.0, 3.0, 2.0], [80.0, 3.0, 3.5]]}
    ...
    if args.scaffold:
        scaf_condition = ['O=C(Cc1ccccc1)NCc1ccccc1', 'c1cnc2[nH]ccc2c1', 'c1ccc(-c2ccnnc2)cc1',
    ...
    if prop_condition is None and scaf_condition is None:
        ...
        for i in tqdm(range(gen_iter)):
            ...
            sca_ = torch.tensor([stoi[s] for s in regex.findall(sca)], dtype=torch.long)[None, ...].repeat(
                args.batch_size, 1).to(device)
            y = sample_scaffold(model, x, args.block_size, temperature=tempera, sample=True, top_k=None, prop=p,
                                scaffold=sca_)
            # y = sample(model, x, args.block_size, temperature=tempera, sample=True, top_k=None, prop=p,
            #            scaffold=sca)  # 0.7 for guacamol 1 for moses
            for gen_mol in y:
                ...
                for i in tqdm(range(gen_iter)):
                    ...
                    if len(args.props) == 1:
                        ...
                        p = torch.tensor([c]).repeat(args.batch_size, 1).unsqueeze(1).to(
                            device)  # for multiple conditions
                    sca = torch.tensor([stoi[s] for s in regex.findall(j)], dtype=torch.long)[None, ...].repeat(
                        args.batch_size, 1).to(device)
                    y = sample(model, x, args.block_size, temperature=tempera, sample=True, top_k=None, prop=p,
                               scaffold=sca)  # 0.7 for guacamol
