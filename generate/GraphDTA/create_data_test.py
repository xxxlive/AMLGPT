...
def process_chunk(start_pos, chunk_size, file_path):
    """
    Reads and decompresses a chunk of the file, then deserializes it.
    """
...
def parallel_load_graph_dict(graph_dict_path, num_chunks=64):
    """
    Loads a large graph dict in parallel by splitting the file into chunks.
    """
    # Get the size of the compressed file
    file_size = os.path.getsize(graph_dict_path)

    # Calculate the chunk size
    chunk_size = file_size // num_chunks

    # Prepare the start positions for each chunk
    start_positions = [i * chunk_size for i in range(num_chunks)]
    if start_positions[-1] != file_size:  # Ensure the last chunk reads the rest of the file
        start_positions[-1] = file_size - chunk_size

    # Use a multiprocessing pool to process the chunks
    with Pool(processes=num_chunks) as pool:
        # Map the start positions to the process_chunk function
        results = tqdm(pool.starmap(process_chunk,
                                    [(start_pos, chunk_size, graph_dict_path) for start_pos in start_positions]))

    # Merge the results into a single dictionary
    smile_graph = {}
    for result in results:
        smile_graph.update(result)

    return smile_graph



def serialize_and_compress_chunk(chunk):
    """
    Function to serialize and compress a chunk of the smile_graph dictionary.
    """
    return gzip.compress(pickle.dumps(chunk))


def parallelize_graph_saving(smile_graph, graph_dict_path, num_chunks=64):
    # Split the graph dictionary into smaller chunks
    chunk_size = len(smile_graph) // num_chunks
    chunks = [dict(list(smile_graph.items())[i:i + chunk_size]) for i in
              range(0, len(smile_graph), chunk_size)]

    # Parallelize the serialization and compression of chunks
    with Pool(processes=64) as pool:
        compressed_chunks = tqdm(pool.map(serialize_and_compress_chunk, chunks))

    # Write all compressed chunks to the file
    with open(graph_dict_path, 'wb') as writer:
...
class CustomTestbedDataset(InMemoryDataset):
    def __init__(self, args, root, phase='test', transform=None, pre_transform=None, pre_filter=None):
        ...
        self.root = root
        self.base_data_name = args.smiles_file.split('/')[-1].split('.')[0]

        self.custom_smi_data = args.smiles_file
    ...
    def process_smile(self, smile):
        """
        Function to process each SMILE and generate its graph.
        """
        return smile, smile_to_graph(smile)

    def parallel_encode_load(self, compound_iso_smiles):
        graph_dict_path = os.path.join(self.root, f'{self.base_data_name}_graph_dict.pkl.gz')
        with Pool(processes=64) as pool:
            results = list(tqdm(pool.imap(self.process_smile, compound_iso_smiles), total=len(compound_iso_smiles),
                                     desc="Processing SMILES"))
        smile_graph = dict(results)

        # if os.path.exists(graph_dict_path) and os.path.getsize(graph_dict_path) != 0:
        #     print('Load graph dict from ', graph_dict_path)
        #
        #     # Example usage:
        #     smile_graph = parallel_load_graph_dict(graph_dict_path)

            # with open(graph_dict_path, 'rb') as reader:
            #     smile_graph = pickle.loads(gzip.decompress(reader.read()))

        # else:
        #     print('Generate graph dict to ', graph_dict_path)
        #     # Use multiprocessing Pool to process SMILES in parallel
        ...
        #
        #     # Example usage:
        #     parallelize_graph_saving(smile_graph, graph_dict_path)

            # with open(graph_dict_path, 'wb') as writer:
            #     writer.write(gzip.compress(pickle.dumps(smile_graph)))

        print('SMILES graph generation successful!')
        return smile_graph
    ...
    def process(self):
        ...
        self.df = pd.read_csv(self.custom_smi_data)
        self.df.columns = self.df.columns.str.lower()
        compound_iso_smiles += list(self.df['smiles'])
        compound_iso_smiles = list(set(compound_iso_smiles))

        # if os.path.exists(self.root + f'/{self.base_data_name}_graph_dict.pkl.gz') and os.path.getsize(
        #         self.root + f'/{self.base_data_name}_graph_dict.pkl.gz') != 0:
        #     print('load graph dict from ', self.root + '/graph_dict.pkl.gz')
        #     # with open(self.root + '/graph_dict.pkl', 'rb') as file:
        #     #     smile_graph = pickle.load(file)
        #     with open(self.root + f'/{self.base_data_name}_graph_dict.pkl.gz', 'rb') as reader:
        #
        #         smile_graph = pickle.loads(gzip.decompress(reader.read()))
        # else:
        #     print('generate graph dict to ', self.root + f'/{self.base_data_name}_graph_dict.pkl.gz')
        #     smile_graph = {}
        #     for smile in compound_iso_smiles:
        #         g = smile_to_graph(smile)
        #         smile_graph[smile] = g
        #
        #     with open(self.root + f'/{self.base_data_name}_graph_dict.pkl.gz', 'wb') as writer:
        #         writer.write((gzip.compress(pickle.dumps(smile_graph))))

        smile_graph = self.parallel_encode_load(compound_iso_smiles)
        print('smiles graph generate sucess!')

        # convert Protein Fasta to PyTorch data format
        target = seq_cat(handle_fasta(global_data.get_fasta_path()))

        # assert 1 == 2
        test_drugs = list(self.df['smiles'])
        ...
        # print('smile_graph ', sys.getsizeof(smile_graph) / 1024 / 1024 / 1024, 'GB')
        del smile_graph
        del test_drugs, compound_iso_smiles, self.df
        # print('data_list   ', sys.getsizeof(data_list) / 1024 / 1024 / 1024, 'GB')
