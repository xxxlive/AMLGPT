...
__location__ = os.path.realpath(
    os.path.join(os.getcwd(), os.path.dirname(__file__))) # 获取当前脚本的目录路径
...
# 数据预处理类，用于准备数据以训练生态毒性预测的transformer模型
class PreprocessData():
    ...
    def __GetOneHotEndpoint(self, list_of_endpoints: List[str]):
        ...
        为给定的endpoint构建one-hot编码的numpy数组。将NOEC重命名为EC10。
        '''
    ...
    def ConcatenateOneHotEnc(self, list_of_endpoints: List[str], list_of_effects: List[str], list_of_species_class: List[str]=None):
        '''
        构建与模型训练时完全一致的one-hot编码向量。指定模型应生成预测的endpoints和effects。
    ...
    def get_cid(self, smiles):
        # 通过公有方法调用私有方法
        return self.__GetCID(smiles)